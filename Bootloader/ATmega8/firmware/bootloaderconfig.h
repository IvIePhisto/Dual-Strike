...
*/
	...
	// we need to make sure that PD0 makes the 4066 switch to ps3 so that the usb lines 
	// will go to the atmega8 chip
	PORTD |= (1<<0);
	PORTD &=~(1<<3);
...
// if (start OR (select and home))
#define bootLoaderCondition() ( (!(PINC & (1<<0))) || ( (!(PINC & (1<<1))) &&  (!(PINC & (1<<5))) ))  /* True if jumper is set */
